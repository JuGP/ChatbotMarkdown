...
const markText = (textFont: TextFont[]) => {
  ...
  textFont.forEach(t => {
    ...
    if (boldMatch && italicMatch && !isEndOfLine) text += markdown.iS + markdown.bS + t.ch + markdown.bE + markdown.iE;
    else if (boldMatch && !isEndOfLine) text += markdown.bS + t.ch + markdown.bE;
    else if (italicMatch && !isEndOfLine) text += markdown.iS + t.ch + markdown.iE;
    else text += t.ch;
  })
